...
Downloads and installs Windows Updates from the Microsoft Update Catalog using the PSWindowsUpdate PowerShell module.

.INPUTS

The script does not take any inputs.

.OUTPUTS

No Output other then informational messages.

.EXAMPLE

PS C:\> . Run_WUInstall_On_Boot.ps1

Runs the script directly which starts the update process.
...
To run the script automatically on start-up, so that it runs again after a reboot:
...
To run the script manually from a batch file (right-click > Run as Admin) located in the same folder as the PS script:
...
# Only included here as a sample on how to hide specific updates by using KB artile numbers:
